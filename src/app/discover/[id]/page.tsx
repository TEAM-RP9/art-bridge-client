  ...
  return (
    ...
    >
      ...
      <div className="p-3">
        ...
        {artwork.creationYear && <p className="text-xs text-muted-foreground">{artwork.creationYear}</p>}
      </div>
    </Link>
  );
}
...
export default function ArtworkDetailPage({ params }: ArtworkDetailPageProps) {
  ...
  const dimensionLabel =
    artwork.width && artwork.height
      ? `${artwork.width}${artwork.dimensionUnit ?? ""} × ${artwork.height}${artwork.dimensionUnit ?? ""}`
      : null;
          ...
          <div className="flex flex-col gap-5">
            ...
            <div className="grid grid-cols-2 gap-4">
              {artwork.creationYear && (
                <MetaItem
                  label="Year"
                  value={String(artwork.creationYear)}
                  icon={
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
